...
let updateJson = gameData => {
  return gameData.map(dataGameObj => {
    ...
    let popularity = dataGameObj.hasOwnProperty("popularity")
      ? dataGameObj.popularity
      : null;
    let cover = dataGameObj.hasOwnProperty("cover")
      ? dataGameObj.cover.url
      : null;
    let hypes = dataGameObj.hasOwnProperty("hypes") ? dataGameObj.hypes : null;
    let summary = dataGameObj.hasOwnProperty("summary")
      ? dataGameObj.summary
      : null;
...
module.exports = {
  searchGames: (req, res) => {
    ...
    axios
      .get(
        `https://api-endpoint.igdb.com/games/?search=${query}&limit=39&fields=name,popularity,cover,hypes,summary`,
        {
          headers: {
            "user-key": API_KEY,
  ...
  searchPopularGames: (req, res) => {
    axios
      .get(
        "https://api-endpoint.igdb.com/games/?fields=*&order=popularity:desc&limit=39",
        {
          headers: {
            "user-key": API_KEY,
            Accept: "application/json"
  ...
  addGame: (req, res) => {
    let { name, id, cover, popularity, hypes, summary } = req.body;
